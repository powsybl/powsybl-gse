...
for /f "delims=" %%x in ('type "%installDir%\etc\itools.conf"') do (
  set "var=%%x"
...
if exist %itools_config_dir%\logback-gse.xml ( 
  set options=%options%%itools_config_dir%
) else ( 
  set options=%options%%installDir%\etc
)
set options=%options%\logback-gse.xml"

if "%java_xmx%"=="" ( set java_xmx=8G )

"%JAVA_BIN%" -Xmx%java_xmx% -cp "%installDir%\share\java\*" %options% com.powsybl.gse.demo.GseDemo %args%
